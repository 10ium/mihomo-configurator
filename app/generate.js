...
function customRuleProviderNames() {
  return [...new Set(state.rules
    .filter(r => r.type === 'RULE-SET' && CUSTOM_RULE_PROVIDERS[r.payload] && r.payload !== 'telegram')
    .map(r => r.payload))];
}

function appendCustomRuleProvider(y, name, provider) {
  y += `  ${name}:\n`;
  y += `    behavior: ${provider.behavior}\n`;
  y += `    type: ${provider.type || 'http'}\n`;
  if (provider.format) y += `    format: ${provider.format}\n`;
  y += `    url: ${q(provider.url)}\n`;
  if (provider.path) y += `    path: ${q(provider.path)}\n`;
  y += `    interval: ${provider.interval || 86400}\n`;
  return y;
}
...
function generateFresh() {
  ...
  const customProviders = customRuleProviderNames();
  const hasProviders = state.activeCdnProviders.size > 0 || needTelegram || needRuBlocked || geositeProviders.length > 0 || customProviders.length > 0;
  if (hasProviders) {
    y += `rule-providers:\n`;
    ...
    for (const name of customProviders) {
      y = appendCustomRuleProvider(y, name, CUSTOM_RULE_PROVIDERS[name]);
    }
...
function generateFromImported() {
  ...
  const customProvidersImported = customRuleProviderNames();
  const autoGeneratedProviderNames = new Set();
  // geosite providers
  for (const name of geositeProvidersImported) autoGeneratedProviderNames.add(name);
  // custom providers
  for (const name of customProvidersImported) autoGeneratedProviderNames.add(name);
  // CDN providers
  for (const id of state.activeCdnProviders) {
  ...
  for (const name of Object.keys(CUSTOM_RULE_PROVIDERS)) knownAutoNames.add(name);
  ...
  for (const name of customProvidersImported) {
    const provider = CUSTOM_RULE_PROVIDERS[name];
    newRuleProviders[name] = {
    ...
    if (provider.format) newRuleProviders[name].format = provider.format;
    if (provider.path) newRuleProviders[name].path = provider.path;
  }
  for (const id of state.activeCdnProviders) {
